...
REM Run the comparison with fixed script
echo Running direct comparison between BALM-PPI and MINT...
python direct_model_comparison_fixed.py --test_data %TEST_DATA% --balm_checkpoint %BALM_CHECKPOINT% --balm_config %BALM_CONFIG% --mint_checkpoint %MINT_CHECKPOINT% --mint_config %MINT_CONFIG% --output_dir %OUTPUT_DIR% --batch_size 2
